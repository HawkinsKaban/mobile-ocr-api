...
# Function to clean OCR text results
def clean_ocr_text(text):
    """
    Clean OCR text results by removing unwanted characters and normalizing formatting
    ...
    # Replace newlines in the middle of sentences with spaces
    text = re.sub(r'(?<=[a-zA-Z0-9,;:\)])[\n\r]+(?=[a-zA-Z0-9])', ' ', text)
    
    # Fix common OCR errors
    text = re.sub(r'[|]l', 'l', text)  # Replace '|l' with 'l'
    text = re.sub(r'l\[', 'k', text)   # Replace 'l[' with 'k'
    ...
    # Remove stray characters that often appear in OCR output
    text = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]', '', text)
    
    # Remove unwanted characters often misrecognized in OCR
    text = re.sub(r'[®©™]', '', text)
    
    # Normalize spacing around punctuation
    text = re.sub(r'\s+([.,;:!?])', r'\1', text)
    
    # Fix OCR errors with periods
    text = re.sub(r'(?<=[a-zA-Z0-9])\s*,\s*(?=[a-zA-Z0-9])', ', ', text)
    text = re.sub(r'(?<=[a-zA-Z0-9])\s*\.\s*(?=[A-Z])', '. ', text)
...
def extract_pdf_page_with_subprocess(pdf_path, page_num):
    ...
    try:
        ...
        cmd = [
            pdftoppm_exe,
            "-jpeg",
            "-r", "300"  # Increased to 300 DPI for better quality
        ]
...
# Enhanced image optimization for better OCR results
def optimize_image_for_mobile(image_path, max_dimension=2000):
    """
    Resize and optimize image if too large for efficient processing
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            if width > max_dimension or height > max_dimension:
                ...
                img.save(optimized_path, 'JPEG', quality=95)  # Higher quality
...
def preprocess_image(image_path, preprocess_type='advanced'):
    """
    Enhanced preprocessing for better OCR results - optimized for images
    ...
    try:
        ...
        if preprocess_type == 'fast':
            ...
            # 5. Detect and fix skew if needed
            try:
                # Find lines to detect skew
                ...
                if lines is not None and len(lines) > 0:
                    ...
                    if angles:
                        median_angle = statistics.median(angles) * 180 / np.pi
                ...
                logger.warning(f"Skew correction failed: {skew_error}")
...
def perform_ocr(image_path, language='eng', config=''):
    """
    Perform enhanced OCR with improved settings
    ...
    try:
        ...
        # Optimize Tesseract configuration for better results
        # Try multiple page segmentation modes and select best result
        psm_modes = [3, 4, 6]  # 3=auto, 4=single column, 6=single block
        results = []
        confidences = []
        
        for psm in psm_modes:
            # Set config for OCR with specific PSM
            ...
            try:
                ...
                current_confidences = [int(conf) for conf in data['conf'] if conf != '-1']
        ...
        if confidences:
            ...
            avg_confidence = confidences[best_index]
        else:
            # Fallback to first result
            text = results[0] if results else ""
            avg_confidence = 0
        
        # Clean the OCR text result
    ...
    try:
            ...
            try:
                ...
                if page_num is not None and 0 <= page_num < total_pages:
                    ...
                    if direct_text and len(direct_text.strip()) > 50:  # Jika teks cukup bermakna
                        ...
                        cleaned_text = clean_ocr_text(direct_text)
                        logger.info(f"Berhasil mengekstrak teks langsung dari PDF halaman {page_num}")
                        return cleaned_text, time.time() - start_time, 90.0, total_pages
                else:
        ...
        try:
                ...
                if page_num is not None:
                    ...
                    try:
                        ...
                        pages = convert_from_path(
                            pdf_path, 
                            300,  # Increased to 300 DPI for better quality
            ...
            for i, page in enumerate(pages):
                ...
                # Proses gambar
                processed_path = preprocess_image(optimized_path, 'advanced')
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            if optimize and not file_data["is_pdf"]:
                optimized_path = optimize_image_for_mobile(filepath, max_dimension=2000)
    ...
    try:
        ...
        if file_data["is_pdf"]:
            ...
            try:
                ...
                if not text.startswith("Error") and not text.startswith("Tidak dapat") and not text.startswith("Gagal"):
                    ...
                    if image_path:
                        ...
                        processed_path = preprocess_image(image_path, 'advanced')
            ...
            if not file_data.get("processed"):
                # Preprocess image with advanced mode
                processed_path = preprocess_image(file_data["filepath"], 'advanced')
...
def quick_recognize():
    """
    Quick OCR without session - Enhanced for better image recognition
    
    Request (form-data):
        - image: Image file
        - language: OCR language (default: eng)
        - preprocess: Preprocessing type (default, fast, balanced, advanced, quality)
    ...
    if file and allowed_file(file.filename):
        try:
            # Get parameters
            language = request.form.get('language', 'eng')
            preprocess_type = request.form.get('preprocess', 'advanced')  # Default to advanced
            ...
            # Optimize for mobile
            optimized_path = optimize_image_for_mobile(filepath, max_dimension=2000)  # Increased resolution
...
if __name__ == '__main__':
    ...
    # Register Poppler path to environment
    register_poppler_path()
    # Start Flask app
