...
# Custom JSON encoder that formats text without newline characters
class CleanJSONEncoder(json.JSONEncoder):
    def encode(self, obj):
        if isinstance(obj, dict):
            # Process text and summary fields to remove newlines
            if 'text' in obj:
                obj['text'] = self.clean_text_for_json(obj['text'])
            if 'summary' in obj:
                obj['summary'] = self.clean_text_for_json(obj['summary'])
    ...
    def clean_text_for_json(self, text):
        ...
        # Convert newline characters to special placeholders for paragraphs and line breaks
        # These will be converted back when displayed but won't show as "\n" in the JSON
        text = text.replace('\n\n', ' [P] ')  # Paragraph break
        text = text.replace('\n', ' [BR] ')   # Line break
        return text
...
def ocr():
    ...
    Response:
        ...
        - text: Perfectly formatted extracted text with no escape sequences
    ...
    try:
        ...
        # Use the custom JSON encoder to return a clean JSON response
        # that eliminates newline characters in the "text" and "summary" fields
        response = Response(
            json.dumps(response_data, cls=CleanJSONEncoder),
